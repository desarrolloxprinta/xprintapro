...
async function loadTabData(tab) {
  ...
  if (tab === 'proyectos') {
    ...
    try {
      const { data, error } = await adminSupabase.from('blog_posts').select('*').order('published_date', { ascending: false })
      if (error) throw error
      blogList = data || []
      renderBlogTable()
    } catch (err) {
...
/**
 * Renderiza la tabla del Área Técnica (blog_posts)
 */
...
async function submitArticulo(e) {
  ...
  try {
    if (id) {
      const { error } = await adminSupabase.from('blog_posts').update(payload).eq('id', id)
      if (error) throw error
    } else {
      const { error } = await adminSupabase.from('blog_posts').insert([payload])
      if (error) throw error
    }
